/**
 * DecryptReveal — WebGL2 Holographic ASCII Cipher Engine
 * Matches live UI contrast & shapes to ASCII glyphs, revealed under pointer sweep.
 */
...
export interface DecryptRevealElements {
  ...
  capture?: (dpr: number) => HTMLCanvasElement | null;
}
...
export function createDecryptReveal(
  ...
  const { source, content, output, capture } = elements;
  ...
  const canCapture = typeof capture === "function";
  ...
  let colorKey = "";
  let fg: [number, number, number] = [0.96, 0.62, 0.07];
  ...
  function uploadContent() {
    if (!canCapture || !contentDirty) return;

    let texture: HTMLCanvasElement = source;
    if (capture) {
      const dpr = output.width / Math.max(output.clientWidth, 1);
      const painted = capture(dpr);
      if (!painted) return;
      texture = painted;
    }
  ...
  function render() {
    ...
    gl!.uniform1f(u.radius, Math.max(config.radius, 1));
    ...
    gl!.uniform1f(u.uCrisp, canCapture ? 0 : 1);
    gl!.bindFramebuffer(gl!.FRAMEBUFFER, null);
  ...
  function frame(now: number) {
    ...
    if (
      settled &&
      !contentDirty &&
      (reducedMotion || !canCapture || !churning)
    ) {
  ...
  listenTarget.addEventListener("pointerleave", onPointerLeave as EventListener, { passive: true });
