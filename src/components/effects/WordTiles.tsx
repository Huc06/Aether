...
const nextDelay = () => SHUFFLE_MIN + Math.random() * (SHUFFLE_MAX - SHUFFLE_MIN);
...
> = ({ sentence, className = '', ...props }) => {
  ...
  const swatches = useRef<(Swatch | undefined)[]>([]);

  const reroll = useCallback((index: number) => {
  ...
  return (
    <div
      ...
      role="heading"
      aria-level={1}
      className={`inline-flex max-w-full overflow-hidden rounded-xl align-middle font-sans font-semibold tracking-tight text-[clamp(1.15rem,3.4vw,2.1rem)] leading-none ${className}`}
      {...props}
    >
      {words.map((word, i) => {
